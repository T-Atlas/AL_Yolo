...
class PIDController:
    ...
    def __init__(self, p_gain, i_gain, d_gain, output_limits=(None, None)):
        """
        初始化PID控制器，并设置比例增益、积分增益和微分增益。
        ...
        self.integral_x = 0  # X方向的误差积分
        self.integral_y = 0  # Y方向的误差积分
        self.last_error_x = 0  # 上一次的X方向误差
        self.last_error_y = 0  # 上一次的Y方向误差
    ...
    def update(self, error_x, error_y):
        ...
        output_x = (self.p_gain * error_x) + (self.i_gain * self.integral_x) + (self.d_gain * derivative_x)
        output_y = (self.p_gain * error_y) + (self.i_gain * self.integral_y) + (self.d_gain * derivative_y)
...
if __name__ == "__main__":
    ...
    pid = PIDControllerAdvanced(p_gain=0.1, i_gain=0.01, d_gain=0.05, output_limits=(-100, 100))
    error_x, error_y = 10, -5  # 示例当前值
    output = pid.update(error_x, error_y)
    print("PID输出:", output)
